...
public class ConnectionUtil {
	...
	public static boolean validate(String name, String sha1, int size) throws Throwable {
		...
		try (DigestInputStream stream = new DigestInputStream(Files.newInputStream(pathtofile), digest)) {
			...
			return sha1result.equals(sha1);
		}
	}

	// This attempts to download a file if it isn't valid
	public static void validateDownloadRetry(String url, String name, String sha1, int size) throws Throwable {
		byte times = 0;
		while (!ConnectionUtil.validate(name, sha1, size)) {
