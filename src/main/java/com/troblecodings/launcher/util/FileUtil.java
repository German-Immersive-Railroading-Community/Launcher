...
//        return true;
    ...
    public static class SettingsData {
        ...
        public final String baseDir = getConfigDir() + "/gir";
        ...
        public static final Path SETTINGSPATH = Paths.get(getConfigDir(), "gir", "/settings.json");

        private static String getConfigDir() {
            switch (StartupUtil.OSSHORTNAME) {
                case "windows":
                    return System.getenv("APPDATA");

                case "linux":
                    String env = System.getenv("XDG_CONFIG_DIR");

                    // IF XDG_CONFIG_DIR is null, this means that we should look for `.config/`.
                    // A null env value means that the XDG_CONFIG_DIR value is the default value.
                    if (env == null) {
                        env = System.getProperty("user.home") + "/.config";
                    }
    ...
    public static void readSettings() {
        try {
            if (Files.exists(SettingsData.SETTINGSPATH)) {
                Reader reader = Files.newBufferedReader(SettingsData.SETTINGSPATH);
                SETTINGS = GSON.fromJson(reader, SettingsData.class);
                reader.close();
            } else {
                Files.createDirectories(SettingsData.SETTINGSPATH.getParent());
                Files.createFile(SettingsData.SETTINGSPATH);

                Writer writer = Files.newBufferedWriter(SettingsData.SETTINGSPATH);
