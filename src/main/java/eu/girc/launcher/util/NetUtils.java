...
public final class NetUtils {
     ...
     * @return Either the String body, or an empty Optional if the status code not
     *         200.
     * @throws IOException
    ...
    private static boolean validateSha1(String expectedSha1, Path filePath) throws IOException {
        ...
        final HashCode sha1Hc = sha1.newHasher()
                .putBytes(Files.toByteArray(filePath.toFile()))
                .hash();
    ...
    private static boolean validateSha256(String expectedSha256, Path filePath) throws IOException {
        ...
        final HashCode sha256Hc = sha256.newHasher()
                .putBytes(Files.toByteArray(filePath.toFile()))
                .hash();
    ...
    public static Optional<Path> validateOrDownloadSha1(final URI uri, final Path path, final String sha1)
            throws IOException, InterruptedException {

        logger.debug("Validating if {} exists and SHA-1 matches", path);
        if (!validateSha1(sha1, path)) {
            logger.debug("Failed validation", path);
            return downloadFile(uri, path);
        }

        logger.debug("Succeeded validation", path);
        return Optional.of(path);
    }

    public static Optional<Path> validateOrDownloadSha256(final URI uri, final Path path, final String sha256)
            throws IOException, InterruptedException {
