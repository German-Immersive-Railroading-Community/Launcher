...
public class StartupUtil {
    ...
    public static BetaInfo[] getBetaVersions(boolean refreshBetaData) {
        if (!Launcher.getBetaMode()) return new BetaInfo[0];
        ...
        try {
            ...
            if (refreshBetaData || !Files.exists(betaJsonPath)) refreshBetaJson();
    ...
    public static boolean isJavaAnd8(Path pathToDictionary) {
        ...
        if (Files.notExists(pathtoJava)) return false;
        try {
            ...
            if (versionSplit[1].startsWith("1.8.0")) return true;
        } catch (Exception e) {
            Launcher.onError(e);
        }
        return false;
    ...
    public static Optional<String> findJavaVersion() {
        ...
        Optional<String> opt1 = Arrays.stream(pathVar.split(";")).filter(str -> {
            ...
            if (Files.notExists(pathto)) return false;
        ...
        if (home != null) {
            ...
            if (isJavaAnd8(pathToHome)) return Optional.of(pathToHome.toString());
        }
    ...
    public static void update() {
        try {
            ...
            Files.copy(pth, tempFilePath, StandardCopyOption.REPLACE_EXISTING);
            ...
            if (!ConnectionUtil.openConnection(downloadURL, stream, bytesize -> progress.setProgress(bytesize.intValue()))) {
                stream.close();
                Files.copy(tempFilePath, pth, StandardCopyOption.REPLACE_EXISTING);
                return;
            }
            stream.close();
    ...
    private static String[] prestart() {
        try {
            // call this first thing when starting, so that we most definitely already have the
            ensureServersDatExists();
            ...
            if (!Files.exists(indexes)) Files.createDirectories(indexes);

            String indexpath = indexes + "/" + assetIndex.getString("id") + ".json";
            ...
            Path ogMC = Paths.get(FileUtil.SETTINGS.baseDir + "/versions/" + object.getString("inheritsFrom") + "/" + object.getString("inheritsFrom") + ".jar");
            ...
            if (!Files.exists(ojectspath)) Files.createDirectories(ojectspath);
            ...
            for (Object libentry : arr) {
                ...
                if (downloadobj.has("artifact")) {
                    ...
                    libsBuilder.append(name).append(";");
                    ...
                    if (natives.has(OSSHORTNAME)) {
                        String nativekey = natives.getString(OSSHORTNAME).replace("${arch}", System.getProperty("sun.arch.data.model"));
            ...
            objects.keySet().forEach((key) -> {
                try {
                    ...
                    Path folderpath = Paths.get(ojectspath + "/" + folder);
            ...
            additional.keySet().forEach(key -> additional.getJSONArray(key).forEach(fileobj -> {
                ...
                if (activeBeta != null && name.toLowerCase().contains(activeBeta.getModName())) {
                    ...
                    ConnectionUtil.validateDownloadRetry(jfileobj.getString("url"), path.toString(), jfileobj.getString("sha1"), in -> Footer.setProgress((counter.get() + in) / max));
                }
            ...
            if (serverListTag.size() > 1) {
                while (serverListTag.size() > 1) serverListTag.remove(1);
            }
    ...
    public static Process start() {
        ...
        if (!Files.exists(Paths.get(javaVersionPath)) || !javaVersionPath.endsWith("java.exe")) javaVersionPath = "java";
        ...
        String[] preparameter = new String[]{javaVersionPath, "-Xmx" + ram + "M", "-Xms" + ram + "M", "-XX:HeapDumpPath=MojangTricksIntelDriversForPerformance_javaw.exe_minecraft.exe.heapdump", "-Djava.library.path=" + FileUtil.LIB_DIR, "-cp", LIBPATHS, MAINCLASS, "-width", width, "-height", height};
        ProcessBuilder builder = new ProcessBuilder(Stream.concat(Arrays.stream(preparameter), Arrays.stream(parameter)).toArray(String[]::new));
